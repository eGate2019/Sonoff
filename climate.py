...
class XClimateTH(XEntity, ClimateEntity):
    ...
    _attr_hvac_modes = [HVACAction.OFF, HVACAction.HEATING, HVACAction.COOLING, HVACAction.DRYING]
    _attr_max_temp = 99
    _attr_min_temp = -40
    ...
    def set_state(self, params: dict):
        if "targets" in params:
            ...
            if params["deviceType"] == "normal":
                ...
                self._attr_hvac_mode = HVACAction.DRYING
            elif self.is_aux_heat:
                self._attr_hvac_mode = HVACAction.HEATING
            else:
                self._attr_hvac_mode = HVACAction.COOLING

        try:
            if self.hvac_mode != HVACAction.DRY:
    ...
    async def async_set_hvac_mode(self, hvac_mode: str) -> None:
        if hvac_mode == HVACAction.HEATING:
            params = {
                "mainSwitch": "on",
                "deviceType": "temperature",
                "targets": self.get_targets(True),
            }
        elif hvac_mode == HVACAction.COOLING:
            params = {
                "mainSwitch": "on",
                "deviceType": "temperature",
                "targets": self.get_targets(False),
            }
        elif hvac_mode == HVACAction.DRYING:
            params = {
                "mainSwitch": "on",
                "deviceType": "humidity",
    ...
    async def async_set_temperature(
        ...
        **kwargs
        ...
        if hvac_mode is None:
            params = {}
        elif hvac_mode == HVACAction.HEATING:
            heat = True
...
class XClimateNS(XEntity, ClimateEntity):
    ...
    _attr_entity_registry_enabled_default = False
    _attr_hvac_modes = [HVACAction.OFF, HVACAction.IDLE, HVACAction.HEATING]
    _attr_max_temp = 31
    _attr_min_temp = 16
    _attr_temperature_unit = UnitOfTemperature.CELSIUS
    ...
    def set_state(self, params: dict):
        ...
        if "HMI_ATCDevice" in params and "etype" in params["HMI_ATCDevice"]:
            if cache["HMI_ATCDevice"]["etype"] == "cold":
                self._attr_hvac_modes[1] = HVACAction.COOLING
            else:
                self._attr_hvac_modes[1] = HVACAction.HEATING

        if "ATCEnable" in params or "ATCMode" in params:
            if cache["ATCEnable"]:
                if cache["ATCMode"]:
                    self.set_hvac_attr(HVACAction.AUTO)
                else:
                    self.set_hvac_attr(self._attr_hvac_modes[1])
            else:
                self.set_hvac_attr(HVACAction.OFF)
    ...
    def set_hvac_attr(self, hvac_mode: str) -> None:
        if hvac_mode == HVACAction.IDLE:
            self._attr_hvac_mode = hvac_mode
            self._attr_supported_features = 0
        elif hvac_mode == HVACAction.OFF:
            self._attr_hvac_mode = hvac_mode
            self._attr_supported_features = ClimateEntityFeature.TARGET_TEMPERATURE
        elif hvac_mode in (HVACAction.COOLING, HVACAction.HEATING, HVACAction.IDLE):
            self._attr_hvac_mode = self._attr_hvac_modes[1]
            self._attr_supported_features = ClimateEntityFeature.TARGET_TEMPERATURE

    @staticmethod
    def get_params(hvac_mode: str) -> dict:
        if hvac_mode == HVACAction.AUTO:
            return {"ATCEnable": 1, "ATCMode": 1}
        elif hvac_mode in (HVACAction.COOLING, HVACAction.HEATING):
            return {"ATCEnable": 1, "ATCMode": 0}
        elif hvac_mode == HVACAction.IDLE:
...
class XThermostat(XEntity, ClimateEntity):
    ...
    _attr_hvac_modes = [HVACAction.OFF, HVACAction.HEATING, HVACAction.IDLE]
    _attr_max_temp = 45
    _attr_min_temp = 5
    ...
    async def async_set_temperature(
        ...
        **kwargs
        ...
        if hvac_mode is None:
            params = {}
        elif hvac_mode is HVACAction.OFF:
            params = {"switch": "off"}
        else:
            i = self.hvac_modes.index(hvac_mode)
